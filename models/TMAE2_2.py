...
from layers.Conv_Blocks import MultiScaleAugmentation2, MultiScaleAugmentationBack
...
class TMAE2(nn.Module):
    def __init__(self, configs):
        ...
        self.multi_scale_augmentation = MultiScaleAugmentation2(channels = self.channels,
                                                               num_kernels = 2, 
                                                               init_weight = True)
    ...
    def forward_preprocessing(self, x, x_mark):
        ...
        # MultiScale list
        out_list, x_mark_out_list = self.multi_scale_augmentation(out, x_mark_out)
        breakpoint()
        # embedding        
        x_enc, n_vars = self.embedding(out, x_mark_out)
        
        padded_input, hw_list = self.padding_for_patching(multi_scale_list, self.patch_size)
    ...
    def padding_for_patching(self, input_list, patch_size):
        out_list = []
        hw_list = []
        for x in input_list:
            B, N, H, W = x.size()
