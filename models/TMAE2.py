...
class TMAE2(nn.Module):
    def __init__(self, configs):
        ...
        # --------------------------------------------------------------------------
        self.final_layer = nn.Linear(configs.dec_d_model, self.channels)
    ...
    def forward_preprocessing(self, x, x_mark):
        ...
        if x_mark is not None:
            if self.window_size % period != 0:
                ...
                x_mark_out = rearrange(x_mark.permute(0,2,1), 'batch dimension ( frequency period ) -> batch dimension frequency period', 
                                period = period)
        ...
        # MultiScale list
        # multi_scale_list = self.multi_scale_augmentation(x_enc)

        padded_input, hw_all, hw_origin = self.padding_for_patching(x_enc, self.patch_size)
        patched_input = self.patching(padded_input)
        latent, mask, ids_restore = self.forward_encoder(patched_input, self.mask_ratio) # [B, patch_num, d_model]
        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        unpatched = self.unpatchify(pred, hw_all)
        unpadded = unpatched[:,:,:hw_origin[0],:hw_origin[1]]
        outputs = rearrange(unpadded, 'B D H W -> B D (H W)')[:,:,:T]
        
        outputs = self.final_layer(outputs.permute(0,2,1))
    ...
    def padding_for_patching(self, x, patch_size):
        B, N, H, W = x.size()
        ...
        out = padding_for_patch(x)
        return out, (H+length_H,W+length_W), (H, W)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            x_enc = self.forecast(x_enc, x_mark_enc)
            return x_enc
        if self.task_name == 'imputation':
            x_enc = self.imputation(x_enc, x_mark_enc)
            return x_enc  
        if self.task_name == 'anomaly_detection':
            x_enc = self.anomaly_detection(x_enc, x_mark_enc)
            return x_enc  
        if self.task_name == 'classification':
            x_enc = self.classification(x_enc, x_mark_enc)
            return x_enc
        return None
