...
class VQVAEDecBlock(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.block = nn.Sequential(
            nn.ConvTranspose2d(in_channels, out_channels, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),
            nn.BatchNorm2d(out_channels),
            nn.LeakyReLU(inplace=True))
    ...
    def __init__(self,
        ...
        self.decoder = nn.Sequential(
            ...
            *[VQVAEDecBlock(d, d) for _ in range(int(np.log2(downsample_rate)) - 1)],
            nn.ConvTranspose2d(d, num_channels, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),
            nn.ConvTranspose2d(num_channels, num_channels, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),  # one more upsampling layer is added not to miss reconstruction details
        )
