...
from layers.Conv_Blocks import conv_resize, conv_resizeback
...
class TimesMaskingBlock(nn.Module):
    def __init__(self, configs):
        ...
        self.conv_resize = conv_resize(in_channels = configs.d_model, out_channels = configs.d_model)

        self.conv_resizeback = conv_resizeback(in_channels = configs.d_model, out_channels = configs.d_model)
    ...
    def forward(self, x):
        ...
        for i in range(self.k):
            ...
            out_high, out_middle = self.conv_resize(out)
            masked_out_high, masked_out_middle = self.mask_specific_size(out_high), self.mask_specific_size(out_middle)
            B, D, Hh, Wh = masked_out_high.size()
            B, D, Hm, Wm = masked_out_middle.size()
            masked_high_flatten, masked_middle_flatten = masked_out_high.reshape(B, -1, D), masked_out_middle.reshape(B, -1, D)
            high_enc_out, attns = self.encoder(masked_high_flatten, attn_mask=None)
            middle_enc_out, attns = self.encoder(masked_middle_flatten, attn_mask=None)

            high_enc_out, middle_enc_out = high_enc_out.reshape(B, D, Hh, Wh), middle_enc_out.reshape(B, D, Hm, Wm)
            out = self.conv_resizeback(high_enc_out, middle_enc_out)
            # reshape back
