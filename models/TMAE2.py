...
def FFT_for_Period(x, k=2):
    ...
    frequency_list[0] = 0
    # frequency 중 상위 k개 선택 후, 상위 k 개 인덱스로 반환
...
class TimesMaskingBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.k):
            ...
            for i in range(len(conv_output_list)):
                ...
                total_masked_output.append(conv_masked_output_flatten)
            # weight 역산 생각 deconvolution
        ...
        # residual connection
        res = res + x
        return res, total_input, total_masked_output
    ...
    def mask_specific_size(self, input_tensor, mask_size = (3,3), num_masked = 5):
        ...
        # 0 ~ total_regions - 1 만큼 무작위 Inedx 생성 후, num_masked 만큼 뽑기
        random_indices = torch.randperm(total_regions)[:num_masked]
...
class TMAE2(nn.Module):
    def __init__(self, configs):
        ...
        self.model = nn.ModuleList([TimesMaskingBlock(configs) for _ in range(configs.e_layers)])
    ...
    def pretraining(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        for i in range(self.layer):
            enc_out, total_input, total_masked_output = self.model[i](enc_out)
        ...
        dec_out = dec_out + \
                  (means[:, 0, :].unsqueeze(1).repeat(
                      1, self.pred_len + self.window_size, 1))
        return dec_out, total_input, total_masked_output
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        if self.pretrain:
            dec_out, total_input, total_masked_output = self.pretraining(x_enc, x_mark_enc, x_dec, x_mark_dec)
            return dec_out, total_input, total_masked_output
        else:
            if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
                dec_out, total_input, total_masked_output = self.forecast(x_enc, x_mark_enc, x_dec, x_mark_dec)
                return dec_out[:, -self.pred_len:, :], total_input, total_masked_output  # [B, L, D]
            if self.task_name == 'imputation':
                dec_out, total_input, total_masked_output = self.imputation(
                    x_enc, x_mark_enc, x_dec, x_mark_dec, mask)
                return dec_out, total_input, total_masked_output  # [B, L, D]
            if self.task_name == 'anomaly_detection':
                dec_out, total_input, total_masked_output = self.anomaly_detection(x_enc)
                return dec_out, total_input, total_masked_output  # [B, L, D]
            if self.task_name == 'classification':
                dec_out, total_input, total_masked_output = self.classification(x_enc, x_mark_enc)
                return dec_out  # [B, N]
        return None