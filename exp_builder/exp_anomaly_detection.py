...
def training_anomaly_detection(
    ...
    ckp_metric: str, savedir: str, pred_len: int, label_len: int, model_name:str, model_type:str):
    ...
    for epoch in range(epochs):
        ...
        for idx, item in enumerate(trainloader):
            ...
            if (idx+1) % eval_interval == 0:
                eval_metrics = test_anomaly_detection(
                    model         = model, 
...
def test_anomaly_detection(model, dataloader, criterion,  
                           log_interval: int, pred_len: int, label_len: int, model_type: str, 
    ...
    # targets and outputs, Anomaly score
    total_targets = []
    total_outputs = []
    dist = []
